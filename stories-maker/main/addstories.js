...
async function initAddStories() {
    // Add event listeners
    setupAddStoriesListeners();

    // Initialize storage and load existing user stories
    await initStorage();
    await loadUserStories();
}

// Load user stories from localforage
async function loadUserStories() {
    ...
    userStories = await getUserStoriesFromStorage();
    ...
    document.querySelectorAll('.read-story-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            await openUserStoryInReader(userStories[index].id);
        });
    });
...
async function handleEditStorySubmit(e) {
    ...
    if (translationsText) {
        try {
            ...
            await saveDictionaryForStory(storyId, translations);
    ...
    // Save to localforage
    await saveUserStoriesToStorage(userStories);
...
// Process story data - UPDATED to handle multiple stories with localforage
async function processStoryData(storyData, fileName) {
    ...
    if (Array.isArray(storyData)) {
        ...
        for (const story of storyData) {
            const result = await processSingleStory(story, fileName);
            if (result.success) {
        ...
        // Single story object
        const result = await processSingleStory(storyData, fileName);
        if (result.success) {
            processedStories.push(result.story);
            totalImported++;
        } else {
    ...
    // Save all imported stories to localforage
...
async function processSingleStory(storyData, fileName) {
    // Validate story structure
    if (!validateStory(storyData)) {
        return { success: false, reason: 'Invalid format' };
    }

    // Check if story already exists (by title and content) in both arrays
    ...
    const storyId = storyData.id || 'user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    ...
    const userStory = {
        ...
        wordCount: storyData.wordCount || calculateWordCount(storyData.content),
    ...
    // Save translations separately to localforage if they exist
...
async function openUserStoryInReader(storyId) {
    // Find the story in local array
    ...
    // Get translations for this story
    let translations = userDictionaries[storyId] || {};
    
    // If not found in local, try from storage
    if (Object.keys(translations).length === 0) {
        translations = await getDictionaryForStory(storyId);
    }
    ...
    const storyPage = '../index.html?id=' + storyId + '&userStory=true';
    window.location.href = storyPage;
}

// Delete user story
async function deleteUserStory(index) {
...
function handleFileSelect(e) {
    ...
    if (extension === 'json' || extension === 'js') {
        ...
        showNotification('Please select a valid JSON or JS file.', 'error');
    }
}
...
function handleFileDrop(e) {
    ...
    if (files.length > 0) {
        ...
        if (extension === 'json' || extension === 'js') {
            ...
            showNotification('Please drop a valid JSON or JS file.', 'error');
        }
    }
}
    ...
    if (extension === 'js') {
        ...
        reader.onload = function (e) {
            try {
                ...
                if (!window.storiesData || !window.storiesData.stories || !Array.isArray(window.storiesData.stories) || window.storiesData.stories.length === 0) {
                ...
                stories.forEach(story => {
                    // Determine cover type based on cover value
                    let coverType = 'emoji';
                    let cover = story.cover || '📚';
                    
                    if (cover && cover.startsWith('data:image')) {
                        coverType = 'image';
                    } else if (cover && (cover.startsWith('http://') || cover.startsWith('https://'))) {
                        coverType = 'url';
                    } else if (cover && cover.startsWith('fas ') || cover && cover.startsWith('fa-')) {
                        coverType = 'icon';
                        if (!cover.startsWith('fas ') && !cover.startsWith('fab ')) {
                            cover = 'fas ' + cover;
                        }
                    } else if (cover && /\p{Emoji}/u.test(cover)) {
                        coverType = 'emoji';
                    } else if (cover) {
                        coverType = 'icon';
                    }

                    // Normalize story structure
                    const normalizedStory = {
                        id: story.id || ('js_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)),
                        ...
                        sound: story.sound || '',
                        cover: cover,
                        coverType: story.coverType || coverType,
                        content: Array.isArray(story.content) ? story.content : [],
                        dictionaries: Array.isArray(story.dictionaries) ? story.dictionaries : [],
                        wordCount: story.wordCount || (story.content ? story.content.join(' ').split(/\s+/).length : 0),
                        translations: story.translations || {}
                    };

                    processStoryData(normalizedStory, file.name);
                    imported++;
                });

                showNotification(`Imported ${imported} story(ies) from JS file!`, 'success');
                clearSelectedFile();
...
function validateStory(story) {
    ...
    if (story.coverType && !['emoji', 'icon', 'image', 'url'].includes(story.coverType)) {
        return false;
    }
...
document.addEventListener('DOMContentLoaded', async function () {
    // Initialize storage and load dictionaries from localforage
    await initStorage();
...
async function deleteAllUserStories() {
    console.log('deleteAllUserStories called, userStories.length:', userStories.length);
    if (userStories.length === 0) {
    ...
    try {
        ...
        // 4. Update localforage
        await saveUserStoriesToStorage([]);
        await saveAllDictionaries({});

        // 5. Also clear localStorage for backward compatibility
        ...
        // 7. Update user stories list
        await loadUserStories();

        // 8. Show success message
        showNotification(`✅ All user stories deleted successfully!`, 'success');
...
function setupAddStoriesListeners() {
    ...
    if (previewBtn) {
        console.log('Preview button found, adding click listener');
