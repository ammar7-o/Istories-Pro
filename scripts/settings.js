...
function performFullReset() {
    ...
    const defaultSettings = {
        ...
        // Other
        customCSS: '',
        customJS: '',  // Add custom JS default
        ttsEnabled: false
    };
    ...
    localStorage.setItem('customJS', defaultSettings.customJS);  // Add this line
    ...
    // Reset TTS language
    localStorage.setItem('ttsLanguage', 'en');
    ttsLanguage = 'en';
    ...
    // Remove custom JS
    const customJSScript = document.getElementById('custom-js-script');
    if (customJSScript) {
    ...
    // Clear custom JS input if it exists
...
function initTranslationSettings() {
    ...
    languageSelect.addEventListener('change', function () {
        ...
        // Update TTS language notification
        const ttsLang = getTTSLanguage();
        showNotification(`Default language set to: ${getLanguageName(selectedLanguage)}. TTS will use: ${getLanguageName(ttsLang)}`, 'success');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Initialize TTS language settings (NEW)
    initTTSLanguageSettings();
    ...
    const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            if (mutation.target.id === 'dictionaryPopup' &&
                ...
                // Update TTS language indicator (NEW)
                updateTTSLanguageIndicator();
...
// =============google tts =================
// Initialize TTS language settings
function initTTSLanguageSettings() {
    const ttsLanguageSelect = document.getElementById('ttsLanguageSelect');

    if (!ttsLanguageSelect) {
        console.warn('TTS language select element not found');
        return;
    }

    // Load saved TTS language from localStorage
    const savedTTSLanguage = localStorage.getItem('ttsLanguage') || 'en';
    ttsLanguageSelect.value = savedTTSLanguage;
    ttsLanguage = savedTTSLanguage;

    // Save TTS language when changed
    ttsLanguageSelect.addEventListener('change', function () {
        const selectedLanguage = this.value;
        localStorage.setItem('ttsLanguage', selectedLanguage);
        ttsLanguage = selectedLanguage;

        // Show notification
        showNotification(`TTS language set to: ${getLanguageName(selectedLanguage)}`, 'success');

        // Update the TTS indicator if dictionary is open
        if (document.getElementById('dictionaryPopup').style.display === 'block') {
            updateTTSLanguageIndicator();
        }
    });

    console.log('TTS language settings initialized with:', savedTTSLanguage);
}
// Update the TTS language indicator
function updateTTSLanguageIndicator() {
    const dictionaryHeader = document.querySelector('.dictionary-word-container');
    if (!dictionaryHeader) return;

    // Remove existing indicator if any
    const existingIndicator = document.getElementById('tts-language-indicator');
    ...
    indicator.addEventListener('click', function () {
        ...
        // Scroll to TTS language section
        setTimeout(() => {
            const ttsSelect = document.getElementById('ttsLanguageSelect');
            if (ttsSelect) {
                ttsSelect.scrollIntoView({ behavior: 'smooth', block: 'center' });
                ttsSelect.focus();
            }
        }, 400);
    });

    dictionaryHeader.appendChild(indicator);
}
function getTTSLanguage() {
    // First try to get the dedicated TTS language
    const ttsLang = localStorage.getItem('ttsLanguage');
    if (ttsLang) {
        return ttsLang;
    }

    // Fallback to translation language if no TTS language is set
    const selectedLang = localStorage.getItem('defaultTranslateLanguage') || 'ar';
    const languageMap = {
        'ar': 'ar',      // Arabic
        'fr': 'fr',      // French
        'es': 'es',      // Spanish
    ...
    return languageMap[selectedLang] || 'en';
}

// Update the playGoogleVoice function to use the selected language
function playGoogleVoice(word) {
    ...
    showNotification(`Speaking in ${getLanguageName(language)}`, 'info', 1000);

    // Google TTS URL with selected language
    ...
    audio.play()
        .then(() => {
            console.log(`Playing in ${language}: ${text}`);
        })
        .catch(error => {
    ...
    audio.onerror = () => {
        // If proxy fails, try native
        useNativeSpeechSynthesis(text, language);
        resetTTSButton();
    };
}
function addTTSLanguageIndicator() {
    updateTTSLanguageIndicator();
}
...
// Update the native speech synthesis fallback
function useNativeSpeechSynthesis(text, language) {
    ...
    if (voices.length) {
        ...
        utterance.voice = voices.find(v => v.lang === langCode) ||
...
// Helper function to extract all words from story content
function extractStoryWords(content) {
    if (!content || !Array.isArray(content)) return [];

    const words = new Set();
    const wordRegex = /[A-Za-zÀ-ÖØ-öø-ÿ0-9’']+(?:[’'\-][A-Za-zÀ-ÖØ-öø-ÿ0-9]+)*/g;
    ...
    content.forEach(paragraph => {
        ...
        matches.forEach(word => {
            if (word.length > 1) { // Only add words with length > 1
...
async function translateWithAI(text, index, paragraphElement) {
    ...
    try {
        ...
        const translationHTML = `
            ...
            <div class="translation-prg" style="line-height: 1.5; color: var(--text); font-size: 0.9em; 
